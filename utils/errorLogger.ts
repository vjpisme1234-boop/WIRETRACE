...
// Captures console.log/warn/error for AI debugging
      ...
      fetch(url, {
        ...
        if (!fetchErrorLogged) {
          ...
          if (typeof window !== 'undefined' && window.console) {
            (window.console as any).__proto__.log.call(console, '[WireTrace] Fetch error (will not repeat):', e.message || e);
          }
        }
      });
    } catch (e) {
...
export const setupErrorLogging = () => {
  ...
  originalConsoleLog('[WireTrace] Setting up error logging...');
  originalConsoleLog('[WireTrace] Log server URL:', logServerUrl || 'NOT AVAILABLE');
  originalConsoleLog('[WireTrace] Platform:', Platform.OS);
